...
export function StatusCard({ isVisible }: { isVisible: boolean }) {
  ...
  const [latencyTextColors, setLatencyTextColors] = useState<
  ...
  useEffect(() => {
    if (dbInfo) {
      ...
      const latencyTextColorsVar: Record<string, string> = {
        ...latencyTextColors,
      };
      ...
      {isVisibleDelayed && (
        <Card
          ...
          className={`${classes.card} ${
            isVisible ? classes.slideInLeft : classes.slideOutLeftClass
          }`}
        >
                    ...
                    <Card.Section display={"flex"} mt={-5}>
                      ...
                      {nodeData.latency && (
                        <Text
                          mt={1}
                          size="13px"
                          color={latencyTextColors[nodeId]}
                        >
                          {nodeData.latency}ms
                        </Text>
                      )}
